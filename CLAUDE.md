<!-- GENERATED — do not edit here.
     Source: tuttiai/knowledge → standards/base-claude.md + projects/tutti-python/CLAUDE.md
     Regenerate: node scripts/sync-claude.mjs tutti-python
 -->
...
# Project: tutti-python

An early-stage Python SDK. Packaging, CI and the publish workflow are in place; the runtime
is not yet built out.

The TypeScript standards above mostly do not apply here. What does: UK English, Conventional
Commits, no automated-authorship trailers, and honesty about state.

## Scope

This repo is a placeholder — the packaging, CI and publish workflow are real; the implementation
is stubs. Check the current direction in the knowledge base before building features here, and
prefer an HTTP client for `@tuttiai/server` over a second runtime: duplicating the security
guards in another language duplicates the attack surface in the area the project markets on.

## Python conventions

- `requires-python = ">=3.9"`. That floor rules out PEP 604 unions in annotations without
  `from __future__ import annotations`, and `match`. Worth revisiting — 3.11+ is defensible for a
  project with no users.
- **Strict mypy.** Do not relax it. It is the Python equivalent of the framework's `strict: true`,
  and the reason to keep it is the same.
- Ruff, line length 88.
- pytest with `asyncio_mode = "auto"` — async-first, with sync wrappers where useful.
- Provider SDKs are **extras** (`anthropic`, `openai`, `all`), never hard dependencies.

## Publishing

`publish.yml` exists and 0.0.1 is on PyPI. As with npm, **do not publish from an agent** — that is
a human, credential-gated action.
